...
return Def.ActorFrame {
    ...
    Def.Sprite {
        ...
        InitCommand=function(self)
            self:zoomx(2):zoomy(.5):z(offsetChart[sButton]):animate(false):diffuse(color(RSColorTable[sButton])):setstate(0):fadebottom(0.2)
    ...
    Def.Sprite {
        ...
        Texture='sprites/glinewave 8x1.png',
        InitCommand=function(self)
            self:zoomx(2):zoomy(.5):z(offsetChart[sButton]):animate(false):diffuse(color(RSColorTable[sButton])):setstate(0):fadebottom(0.2):x(-160)
            if tonumber(sEffect) > 0 then
                self:diffuse(1,1,1,1)
            end
        end,
