...
return Def.ActorFrame {
    Def.Model { --glass center
        InitCommand=function(self)
            self:rotationx(90):diffuse(.1,.1,.1,1)
            if string.find(sButton, "Strum") then
                self:zoom(1.25):diffuse(color(RSFColorTable[sButton]))
            else
    ...
    Def.Model { --glow effect
        InitCommand=function(self)
            ...
            if not string.find(sButton, "Strum") then
                self:zoomx(1.5):zoomy(1.2):zoomz(1.2):z(-3)
            else
                self:zoomx(9):zoomy(1.5):zoomz(1.5):z(3):diffusealpha(0.9)
            end
    ...
    Def.ActorFrame { --light rings
        ...
        Def.Model {
            InitCommand=function(self)
                self:x(15):diffuse(color(RSFColorTable[sButton]))
            end,
        ...
        Def.Model {
            InitCommand=function(self)
                self:x(-15):diffuse(color(RSFColorTable[sButton])):rotationy(180)
            end,
            Meshes="models/lightring.txt",
            Materials="models/materials/default mats.txt",
            Bones="models/lightring.txt"
    ...
    Def.Model { --base thingy
        Condition=(Var "Button" ~= "Strum Up"),
        InitCommand=function(self)
            self:rotationx(90):backfacecull(false):zoomy(.67):zoomz(.67):zoomx(.8)
        if tonumber(sEffect) > 0 then
