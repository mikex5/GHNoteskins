...
local ExploCom = function(self)
    if string.find(sButton, "Strum") then
        self:stoptweening():z(5):diffuse(color(FHColorTable[sButton])):zoom(1):linear(.1):z(50):zoom(.5):decelerate(.04):z(70):diffusealpha(0):zoom(.3)
    else
...
local ExploFlash = function(self)
    if string.find(sButton, "Strum") then
        self:stoptweening():diffuse(color(FHColorTable[sButton])):zoomy(2):zoomx(10):linear(.1):diffusealpha(.7):zoomy(2.5):zoomx(13):decelerate(.04):diffusealpha(0):zoomy(3):zoomx(15)
    else
