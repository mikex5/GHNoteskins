    ...
    return Def.ActorFrame{
        ...
        Def.ActorFrame{ --Highway
            ...
            Def.Quad{ --Highway edges
                InitCommand=function(self)
                    self:xy(-170,-400):zoomto(10,1100):diffuse(Alpha(color(RSQColorTable["Fever"]),0)):fadetop(0.3):blend('BlendMode_Add')
                    self.prevAmount = 0
            ...
            Def.ActorFrame{ --Fever effects
                ...
                Def.Sprite{
                    ...
                    InitCommand=function(self)
                        self:x(-134):diffuse(color(RSQColorTable["Fever"])):zoomx(2):zoomy(4)
                    end
                },
                Def.Sprite{
                    Texture="sprites/feversprite1.png",
                    InitCommand=function(self)
                        self:x(134):zoomx(-2):zoomy(4):diffuse(color(RSQColorTable["Fever"]))
                    end
                },
                FeverMessageCommand=function(self,params)
                    if params.pn ~= sPlayer then return end
            ...
            Def.ActorFrame{ --More fever effects
                ...
                Def.Quad{
                    InitCommand=function(self)
                        self:x(-150):zoomto(32,1100):diffuse(Alpha(color(RSQColorTable["Fever"]),0)):diffuselowerleft(color(RSQColorTable["Fever"]))
                    end
                },
                Def.Quad{
                    InitCommand=function(self)
            ...
            Def.Sprite{ --Even MORE
                InitCommand=function(self)
                    self:y(0):zoomx(10):zoomy(4):diffuse(Alpha(color(RSQColorTable["Fever"]),0))
                end,
            ...
            Def.ActorFrame {
                ...
                Def.Sprite {
                    ...
                    FeverMessageCommand=function(self,params)
                        ...
                        if params.Active then
                            self:stoptweening():diffuse(1,1,1,1):zoom(1.5):linear(0.5):zoom(1.25):diffuse(color(RSQColorTable["Fever"]))
                        else
        ...
        Def.ActorFrame{ --Strum bracket
            ...
            Def.Model {
                InitCommand=function(self)
                    self:x(157):diffuse(color(RSQColorTable["192nd"]))
                end,
            ...
            Def.Model {
                InitCommand=function(self)
                    self:rotationy(180):x(-157):diffuse(color(RSQColorTable["192nd"]))
                end,
                Meshes="models/hammer.txt",
                Materials="models/materials/strum mats.txt",
                Bones="models/hammer.txt",
...
return Def.ActorFrame {
    ...
    Def.Model {
        InitCommand=function(self)
            self:diffuse(color(RSQColorTable["192nd"]))
            RBReceptor[sPlayer][sButton][3] = self
        end,
        PressCommand=function(self) self:glow(Alpha(color(RSQColorTable["192nd"]), 0.2)) end,
    ...
    Def.Sprite { --On press light effect
        ...
        InitCommand=function(self)
            self:diffuse(ColorDarkTone(color(RSQColorTable["192nd"]))):blend('BlendMode_Add'):rotationx(-90):z(3):zoom(1.6):diffusealpha(0)
        end,
