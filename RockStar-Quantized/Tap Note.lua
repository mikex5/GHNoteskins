...
return Def.ActorFrame {
    Def.Model { --glass center
        InitCommand=function(self)
            self:rotationx(90):rotationz(angleChart[sButton]):z(offsetChart[sButton]):diffuse(color(RSQColorTable[sColor]))--:MaterialRate(math.random(8,13)/10)
    ...
    Def.Model { --glow effect
        InitCommand=function(self)
            ...
            if not string.find(sButton, "Strum") then
                self:zoomx(2):zoomy(1.2):zoomz(1.2):z(offsetChart[sButton] - 3)
            else
                self:zoomx(8):zoomy(0.9):zoomz(0.9):z(offsetChart[sButton] + 3):diffusealpha(0.7)
            end
            if tonumber(sEffect) > 0 then
