...
for i = 1,5 do
    Shards[#Shards+1] = Def.ActorFrame{
        InitCommand=function(self)
            self:diffuse(color(RSQColorTable["192nd"])):diffusealpha(0):blend('BlendMode_Add'):xy(math.random(-30,30), 12)
            if i > 4 then
...
return Def.ActorFrame {
    ...
    Def.ActorFrame {
        ...
        Def.Sprite {
            ...
            InitCommand=function(self)
                self:animate(false):diffuse(Alpha(color(RSQColorTable["192nd"]),0)):rotationx(90):z(12):blend('BlendMode_Add')
            end,
