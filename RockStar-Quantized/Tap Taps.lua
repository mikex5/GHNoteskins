...
return Def.ActorFrame {
    Def.Model { --glass center
        InitCommand=function(self)
            self:rotationx(90):rotationz(angleChart[sButton]):z(offsetChart[sButton]):diffuse(.1,.1,.1,1)
            if string.find(sButton, "Strum") then
                self:zoom(1.25):diffuse(color(RSQColorTable[sColor]))
            else
    ...
    Def.Model { --glow effect
        InitCommand=function(self)
            self:rotationx(90):rotationz(angleChart[sButton]):z(offsetChart[sButton]):diffuse(color(RSQColorTable[sColor])):blend('BlendMode_Add')
    ...
    Def.ActorFrame {
        ...
        Def.Model { --end cap #1
            ...
            Materials=(string.find(sButton, "Strum") and "models/materials/Strum hopo mats.txt") or "models/materials/default mats.txt",
        ...
        Def.Model { --end cap #1, strum fever only
            ...
            InitCommand=function(self) self:x(160):diffuse(color(RSQColorTable["Fever"])) end,
        ...
        Def.Model { --end cap #2
            InitCommand=function(self)
                if string.find(sButton, "Strum") then
                    self:x(-160):rotationy(180):zoom(.995)
                else
        ...
        Def.Model { --end cap #2, strum fever only
            Condition=(Var "Button" == "Strum Up" and tonumber(Var "Effect") > 0),
            InitCommand=function(self) self:rotationy(180):x(-160):diffuse(color(RSQColorTable["Fever"])) end,
            Meshes="models/socket.txt",
            Materials="models/materials/Strum hopo mats.txt",
            Bones="models/endcap.txt",
            FeverMissedMessageCommand=function(self,params)
    ...
    Def.ActorFrame { --light rings
        ...
        Def.Model {
            InitCommand=function(self)
                self:x(15):diffuse(color(RSQColorTable[sColor]))
            end,
        ...
        Def.Model {
            InitCommand=function(self)
                self:x(-15):diffuse(color(RSQColorTable[sColor])):rotationy(180)
            end,
            Meshes="models/lightring.txt",
            Materials="models/materials/default mats.txt",
            Bones="models/lightring.txt"
    ...
    Def.Model { --base thingy
        ...
        if tonumber(sEffect) > 0 then
                self:diffuse(Alpha(color(RSQColorTable["Fever"]),.9))
            else
